...
unsigned long zlib_compress_bound(unsigned long source_len) {
    return zng_compressBound(source_len);
}
...
EMSCRIPTEN_KEEPALIVE
double zlib_benchmark_zng_crc32(const char* data, int size, int iterations) {
    if (!data || size <= 0 || iterations <= 0) return -1.0;
    
    double start_time = emscripten_get_now();
    for (int i = 0; i < iterations; i++) {
        volatile uLong result = zng_crc32(0L, (const Bytef*)data, size);
...
double zlib_benchmark_compression(const char* data, int size, int iterations, int level) {
    ...
    uLongf dest_len = zng_compressBound(size);
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        int result = zng_compress2((Bytef*)dest, &current_dest_len, (const Bytef*)data, size, level);
        (void)result; // Prevent optimization
    }
    double end_time = emscripten_get_now();
...
zlib_stream_t* zlib_deflate_init(int level, int window_bits, int mem_level, int strategy) {
    ...
    int ret = zng_deflateInit2(&ctx->stream, level, Z_DEFLATED, window_bits, 
                          mem_level, strategy);
...
int zlib_deflate_process(zlib_stream_t* ctx, const unsigned char* input, 
    ...
    return zng_deflate(&ctx->stream, flush);
}
...
zlib_stream_t* zlib_inflate_init(int window_bits) {
    ...
    int ret = zng_inflateInit2(&ctx->stream, window_bits);
...
int zlib_inflate_process(zlib_stream_t* ctx, const unsigned char* input, 
    ...
    return zng_inflate(&ctx->stream, Z_NO_FLUSH);
}
