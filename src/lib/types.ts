/**
 * Type definitions for zlib-ng.wasm
 * Type-safe interface for next-generation compression with advanced SIMD
 */

// Core WASM module interface for zlib-ng
export interface ZlibNGModule {
  // Optimized compression functions (preferred)
  _zng_compress_optimized?(
    input: number,
    inputLen: number,
    output: number,
    outputLen: number,
  ...
  _zng_compress_bound_optimized?(inputLen: number): number
  _zng_get_version_optimized?(): number
  _zng_crc32_optimized?(crc: number, data: number, len: number): number

  // Standard fallback functions
  _zng_compress_buffer(
    input: number,
    inputLen: number,
    output: number,
    outputLen: number,
  ...
  _zng_compress_bound(inputLen: number): number
  _zng_get_version(): number
  _zng_crc32(crc: number, data: number, len: number): number

  // SIMD-accelerated functions (optimized performance)
  _zng_compress_simd(input: number, input_len: number, output: number, output_len: number, level: number): number
  _zng_crc32_simd_optimized(crc: number, data: number, len: number): number
  _zng_benchmark_simd_compression(data: number, len: number, iterations: number): number
  _zng_simd_capabilities(): number
  _zng_simd_analysis(input: number, input_len: number, compression_ratio: number, simd_speedup: number, memory_efficiency: number): void

  // Optional optimized memory management
  _zng_init_optimized_memory?(): void
  _zng_cleanup_optimized_memory?(): void
